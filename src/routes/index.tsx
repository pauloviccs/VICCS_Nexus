    ...
    <main className="relative min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      <div className="relative z-10 mx-auto max-w-[1360px] px-5 sm:px-6 lg:px-10">
        <header className="flex items-center justify-between py-5 sm:py-6">
          <a href="#catalogo" className="flex items-center gap-3"><span className="lens grid size-10 shrink-0 place-items-center rounded-xl"><span className="glow-core size-3 rounded-full bg-primary" /></span><span className="flex flex-col leading-none"><strong className="font-display text-lg font-semibold">{settings?.hub_name ?? "NEXUS"}</strong><span className="mt-1 text-[10px] uppercase text-muted-foreground">Project hub</span></span></a>
        ...
        <section id="sobre" className="pb-10 pt-6 sm:pt-10"><div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end"><div className="max-w-3xl"><div className="glass mb-6 inline-flex items-center gap-2 rounded-full px-3 py-1"><Sparkles className="size-3 text-primary" /><span className="text-xs text-foreground/70">{settings?.eyebrow ?? "Uma lente viva para cada criação"}</span></div><h1 className="max-w-[18ch] text-balance font-display text-4xl font-semibold leading-[1.02] sm:text-6xl lg:text-7xl">{settings?.headline ?? "Um universo de projetos, reunido em um só lugar."}</h1><p className="mt-6 max-w-2xl text-pretty text-base leading-7 text-muted-foreground lg:text-lg">{settings?.description ?? "Explore sites, sistemas, mods, experiências de web design, código e skills para IDE."}</p></div>
