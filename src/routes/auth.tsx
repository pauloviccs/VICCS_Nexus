...
import { ArrowLeft, Chrome, LoaderCircle, ShieldAlert, Sparkles } from "lucide-react";
...
function AuthPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (mode === "signin") {
      ...
      if (error || !data.user) {
        setLoading(false);
        return setMessage("Não foi possível entrar. Confira seu e-mail e senha.");
      }
    ...
    if (!data.session) {
      ...
      setMessage(
        "Conta criada com sucesso com o cargo comum de 'user'. O acesso ao painel de controle requer aprovação e concessão manual de 'admin' no banco de dados."
      );
    }
  }
    ...
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-background px-5 py-12">
      ...
      <div className="glass-deep relative z-10 w-full max-w-md rounded-2xl p-6 sm:p-8">
        ...
        <form onSubmit={submit} className="mt-7 space-y-4">
          ...
          {message && (
            <div className="flex items-start gap-2.5 rounded-xl border border-destructive/30 bg-destructive/10 p-3.5 text-xs leading-relaxed text-destructive-foreground">
